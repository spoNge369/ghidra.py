...
def main(filename, temp):
    if os.path.isdir(filename):
        return os.system(f'export _JAVA_AWT_WM_NONREPARENTING=1;{GHIDRA_PATH}ghidraRun')
    if '.gpr' in filename:
        os.system(f'export _JAVA_AWT_WM_NONREPARENTING=1;{GHIDRA_PATH}ghidraRun "{os.path.abspath(filename)}"')
        return
    if temp:
